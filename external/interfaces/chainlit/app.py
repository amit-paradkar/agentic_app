...
from entities.core.agent_router.graph import graph_builder
from use_cases.use_cases import ChatUseCases

from external.config.settings import settings

# Global module instances
chat_use_case = ChatUseCases()
...
async def on_message(message: cl.Message):
    ...
    if message.elements:
        for elem in message.elements:
            if isinstance(elem, cl.Image):
                ...
                content = chat_use_case.convert_user_input("image", "text", image_bytes)
    ...
    async with cl.Step(type="run"):
        async with AsyncSqliteSaver.from_conn_string(settings.SHORT_TERM_MEMORY_DB_PATH) as short_term_memory:
            ...
            #output_state = await graph.aget_state({"thread_id": thread_id}, config=config)
...
async def on_audio_end(elements):
    ...
    transcription = chat_use_case.convert_user_input("audio", "text", audio_data)
    #transcription = process_user_query("audio", audio_data)
    ...
    audio_buffer = chat_use_case.convert_user_input("text", "audio", output_state["messages"][-1].content)
